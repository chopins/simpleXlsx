...
class Sheet {
    ...
    public function loadSheet() {
        ...
        $this->sheetXlObj = new \DOMDocument;
        $this->sheetXlObj->load($xl);
        $dimensionTag = $this->sheetXlObj->getElementsByTagName('dimension');
        $dimension = $dimensionTag[0]->getAttribute('ref');
        $this->convertDimension($dimension);
        $this->rows = $this->sheetXlObj->getElementsByTagName('row');
        $this->rowNum = 0;
    }
    ...
    public function readRow() {
        ...
        if ($this->rows->length <= $this->rowNum) {
            return false;
        }
        $doc = $this->rows->item($this->rowNum);
        $this->rowNum++;
