...
<Relationships xmlns="' . $this->docPackageURI . $type . '">';
    ...
    public function load() {
        if ($this->isLoad) {
            throw new Exception("file $this->xlsx has been loaded");
        }
    ...
    public function getSheet($index) {
        ...
        throw new Exception("sheet $index not exists");
    }
    ...
    public function copyXml($file, $xml) {
        copy(dirname(__DIR__) . "/xml/$file", "$this->workspace/$xml");
    }

    public function readTemplateXml($file) {
        return file_get_contents(dirname(__DIR__) . "/xml/$file");
    }
    ...
    protected function createWorkbookRels() {
        ...
        for ($i = 1; $i <= count($this->sheetsArr); $i++) {
            $id = $i + 1;
    ...
    public function zipAddFile($file) {
        $this->zip->addFile("{$this->workspace}/$file", ltrim($file, '/'));
    }

    public function clean() {
        if (is_dir($this->workspace)) {
            $this->rmdir($this->workspace, true);
        }
        rmdir(dirname($this->workspace));
    }
